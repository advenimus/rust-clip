...
- **Clipboard guard re-asserts only on EMPTY, never on different content.** The opt-in guard for nested-VDI scenarios (Mac → VDI → inner remote where the VDI clipboard channel sometimes clears the clipboard right after our write) re-writes the last received text iff `cb.get_text()` returns `Ok("")` or `Err(ContentNotAvailable)`. Any other content — including a brand-new user copy — drops the guard immediately so we never fight a real copy. Re-assertions are capped at 3 per clip (hardcoded `GUARD_MAX_ATTEMPTS` in `sync.rs`) so a runaway clearer can't burn CPU forever, and the re-assert path piggy-backs on the existing `last_set` echo-suppression so it never emits a fresh outbound WS event. Guard logic is extracted into a pure `evaluate_guard` in `clipboard.rs` so the state machine is unit-tested without the OS clipboard.
