...
- **Clipboard guard is tri-state, generalized across content types.** The opt-in guard for nested-VDI scenarios is now `clipboard_guard_mode: off | empty_only | aggressive`. `EmptyOnly` re-asserts only when the clipboard clears (safe in any environment — never fights a real copy). `Aggressive` ALSO re-asserts when the clipboard contents change to a hash that matches a recent inbound clip — the stale-direction-stomp case for Citrix host→guest channels. The guard works for text, images, AND files (one active guard at a time, replaced on every new receive). Per-type recent-inbound ring buffers (8 entries, 30 s TTL) on the worker thread distinguish a stomp from a genuine user re-copy. Inside the post-write quiet windows (3 s images, 5 s files) the guard stays `Keep` rather than reading the clipboard — the RGBA / pasteboard settle ambiguity would otherwise false-positive. When any guard is armed, the worker polls at 100 ms instead of 500 ms so an external channel doesn't get a 500 ms window to stomp. Re-assertions are capped at 3 per clip (hardcoded `GUARD_MAX_ATTEMPTS` in `sync.rs`); the re-assert path piggy-backs on the existing `last_set` echo-suppression so it never emits a fresh outbound WS event. Logic is in pure `evaluate_guard` in `clipboard.rs`, unit-tested across all modes and types. Legacy `clipboard_guard_enabled: bool` in `config.toml` migrates to `clipboard_guard_mode = "empty_only"` on first load.
...
- **Clipboard guard state machine + GuardEntry / evaluate_guard:** `crates/rustclip-client/src/clipboard.rs` (receive-side empty-clipboard re-assert for nested-VDI scenarios).
- **Global re-copy hotkey (registration + handler):** `crates/rustclip-client-gui/src/hotkey.rs`.
